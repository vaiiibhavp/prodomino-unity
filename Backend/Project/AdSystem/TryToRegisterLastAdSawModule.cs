...
public class TryToRegisterLastAdSawModule(ILogger<TryToRegisterLastAdSawModule> logger, IGameApiClient gameApiClient)
{
    private readonly ILogger<TryToRegisterLastAdSawModule> _logger = logger;
    private readonly IGameApiClient _gameApiClient = gameApiClient;

    /// <summary>
    /// Try to update the monthly subscription
    /// </summary>
    [CloudCodeFunction(nameof(TryToRegisterLastAdSaw))]
    public async Task TryToRegisterLastAdSaw(IExecutionContext executionContext)
    {
        ...
        await TryToRegisterLastAdSaw(executionContext, playerAdData, configData);
    }

    /// <summary>
    /// Try to buy the monthly subscription
    /// </summary>
    private async Task TryToRegisterLastAdSaw(IExecutionContext executionContext, 
        PlayerAdData? playerAdData, ConfigData? configData)
    {
        ...
        try
        {
            playerAdData.lastTimeAdSaw = DateTime.UtcNow;
            ...
            _logger.LogInformation("Registered last ad saw time successfully.");
        }
        catch (Exception ex)
        {
