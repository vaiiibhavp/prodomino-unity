...
{
    /// <summary>
    /// Five mode AI logic. Focuses on making the open ends add to a multiple of 5.
    /// </summary>
    public partial class DominoAI
    {
        ...
        /// </summary>
        private int[] GetFiveOpenBranchArray(out List<int> doubleTilesEnabled)
        {
            int rightBranch = 0;
            var leftBranch = 0;
            var topNum = 0;
        ...
        /// </summary>
        public bool IsPlayable_SpecificRules_Five(Domino domino)
        {
            var openValues = GetFiveOpenBranchArray(out var doubles);
            return openValues.Any(x => domino.HasValue(x));
        }

        public Domino[] GetFirstTurnAvailableTilesFive()
        {
            ...
            return GetFirstTurnAvailableTilesFive_CustomHand(aiHand);
        }
        
        public Domino[] GetFirstTurnAvailableTilesFive_CustomHand(List<Domino> hand)
        {
            ...
            DetermineFiveFirstAvailableTiles(ref playable, hand, allBoard);
        ...
        /// <summary>
        /// Determines the best possible move for the Top AI player in Five mode (Pro difficulty),
        /// using a knowledge-constrained profile and an adaptive scoring system.
        /// </summary>
        public Domino GetBestMoveFive()
        {
            var currentTurnControl = GameTurnController.GetCurrentTurnControl();
            var aiHand = GetHandByTurn(currentTurnControl);

            return GetBestMoveFive_CustomHand(aiHand);
        }

        internal Domino GetBestMoveFive_CustomHand(List<Domino> hand)
        {
            ...
            var openValues = GetFiveOpenBranchArray(out var requiredDoublesToOpenBranch);
            ...
            {
                float score = ScoreFiveTile(tile, hand, knownTiles, openValues);
                if (score > bestScore)
                {
                    bestScore = score;
                    bestTile = tile;
        ...
        /// <summary>
        /// Calculates and optionally returns the Five AI scoring probabilities for the player.
        /// </summary>
        /// <param name="onGetProbabilities">Optional callback to receive the calculated probabilities.</param>
        public void DebugFiveAIScoring_Player(Action<Dictionary<Domino, float>> onGetProbabilities = null)
        {
            DetermineProbabilities_Five(1, onGetProbabilities);
        }

        /// <summary>
        /// Logs normalized quality scores for each playable domino in Five mode (Pro),
        /// based on the same logic used by the AI.
        /// </summary>
        public void DebugFiveAIScoring_AI(Action<Dictionary<Domino, float>> onGetProbabilities = null)
        {
            var turn = GameTurnController.GetPlayerTurn()
                ? GameTurnController.GetNextTurn()
                : GameTurnController.GetCurrentTurnControl();

            DetermineProbabilities_Five(turn, onGetProbabilities);
        }

        /// <summary>
        /// Evaluates and determines the probabilities for playable domino tiles on the fifth branch for a given turn,
        /// invoking a callback with the results.
        /// </summary>
        /// <param name="turn">The turn number for which to determine playable domino probabilities.</param>
        /// <param name="onGetProbabilities">Optional callback invoked with a dictionary mapping each playable domino to its calculated probability.</param>
        public void DetermineProbabilities_Five(int turn, Action<Dictionary<Domino, float>> onGetProbabilities = null)
        {
            ...
            if (isFirstTurn)
                DetermineFiveFirstAvailableTiles(ref playable, nextTurnDominos, GetBoardDominos());
            ...
            var scored = GetScoredDominos(playable, tile =>
                ScoreFiveTile(tile, nextTurnDominos, knownTiles, openValues));

            if (scored is { Count: > 0 })
            {
                Debug.Log("AI - Move Evaluation (Five Pro):");
        ...
        /// <summary>
        /// Core scoring logic for evaluating a single domino in Five mode (Pro).
        /// This function is reused by both the decision and debugging logic.
        /// </summary>
        private float ScoreFiveTile(Domino tile, List<Domino> aiHand, List<Domino> knownTiles, int[] openValues)
        {
        ...
        private void DetermineFiveFirstAvailableTiles(ref List<Domino> playableDominos, List<Domino> aiHand, List<Domino> allBoard)
        {
