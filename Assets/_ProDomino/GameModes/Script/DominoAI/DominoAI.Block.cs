...
{
    /// <summary>
    /// Block mode AI logic. Focuses on exhausting numbers to block opponent.
    /// </summary>
    public partial class DominoAI
    {
        ...
        /// </summary>
        private int[] GetBlockOpenBranchArray(out List<int> doubleTilesEnabled)
        {
            int rightBranch = 0;
            var leftBranch = 0;
            var topNum = 0;
        ...
        /// </summary>
        public bool IsPlayable_SpecificRules_Block(Domino domino)
        {
            var openValues = GetBlockOpenBranchArray(out var doubles);
            return openValues.Any(x => domino.HasValue(x));
        }

        public Domino[] GetFirstTurnAvailableTilesBlock()
        {
            ...
            return GetFirstTurnAvailableTilesBlock_CustomHand(aiHand);
        }
        
        public Domino[] GetFirstTurnAvailableTilesBlock_CustomHand(List<Domino> hand)
        {
            ...
            DetermineBlockFirstAvailableTiles(ref playable, hand, allBoard);
        ...
        /// <summary>
        /// Determines the best possible move for the Top AI player in Block mode (Pro difficulty),
        /// using a knowledge-constrained profile and an adaptive scoring system.
        /// </summary>
        public Domino GetBestMoveBlock()
        {
            var currentTurnControl = GameTurnController.GetCurrentTurnControl();
            var aiHand = GetHandByTurn(currentTurnControl);

            return GetBestMoveBlock_CustomHand(aiHand);
        }

        internal Domino GetBestMoveBlock_CustomHand(List<Domino> hand)
        {
            ...
            var openValues = GetBlockOpenBranchArray(out var requiredDoublesToOpenBranch);
            ...
            {
                float score = ScoreBlockTile(tile, hand, knownTiles, openValues);
                if (score > bestScore)
                {
                    bestScore = score;
                    bestTile = tile;
        ...
        /// <summary>
        /// Calculates and optionally returns the Block AI scoring probabilities for the player.
        /// </summary>
        /// <param name="onGetProbabilities">Optional callback to receive the calculated probabilities.</param>
        public void DebugBlockAIScoring_Player(Action<Dictionary<Domino, float>> onGetProbabilities = null)
        {
            DetermineProbabilities_Block(1, onGetProbabilities);
        }

        /// <summary>
        /// Logs normalized quality scores for each playable domino in Block mode (Pro),
        /// based on the same logic used by the AI.
        /// </summary>
        public void DebugBlockAIScoring_AI(Action<Dictionary<Domino, float>> onGetProbabilities = null)
        {
            var turn = GameTurnController.GetPlayerTurn()
                ? GameTurnController.GetNextTurn()
                : GameTurnController.GetCurrentTurnControl();

            DetermineProbabilities_Block(turn, onGetProbabilities);
        }
        ...
        /// based on the same logic used by the AI.
        /// </summary>
        private void DetermineProbabilities_Block(int turn, Action<Dictionary<Domino, float>> onGetProbabilities = null)
        {
            ...
            if (isFirstTurn)
                DetermineBlockFirstAvailableTiles(ref playable, nextTurnDominos, GetBoardDominos());
            ...
            {
                Debug.Log("AI - Move Evaluation (Block Pro):");
        ...
        /// <summary>
        /// Core scoring logic for evaluating a single domino in Block mode (Pro).
        /// This function is reused by both the decision and debugging logic.
        /// </summary>
        private float ScoreBlockTile(Domino tile, List<Domino> aiHand, List<Domino> knownTiles, int[] openValues)
        {
        ...
        private void DetermineBlockFirstAvailableTiles(ref List<Domino> playableDominos, List<Domino> aiHand, List<Domino> allBoard)
        {
