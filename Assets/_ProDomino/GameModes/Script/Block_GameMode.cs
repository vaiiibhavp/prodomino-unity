...
{
    public class Block_GameMode : AbstractGameMode
    {
        ...
        private void Update()
        {
            ...
            if (!IsForcingShowTiles && debugToolsEnabled && !IsDebugingTilesRightNow || IsDebugingTilesRightNow)
            {
                ...
                dominoAI.DebugBlockAIScoring_AI(ShowAIProbabilities);
            }
        ...
        public override void InitializeGameMode(GameModeConfig aux_gameModeConfig, int aux_Difficulty, GameType aux_GameTypeID, NumberPlayers aux_VsPlayerID, bool isSinglePlayerIA, ConcentrateNumberOfTiles auxConcentrateNumberOfTiles, Action openMenuSettingsAction, Func<bool> isTimeOut_MatchManager)
        {
            ...
            extendedGameController.Initialize
                ...
                dominoAI.GetFirstTurnAvailableTilesBlock,
                dominoAI.GetBestMoveBlock,
                () => CurrentDisplayName);
        }
        ...
        public override void SetupRandomHands()
        {
            ...
            else
            {
                ...
                Debug.Log($"Block: Boneyard domino tiles setted. Values: {string.Join(", ", boneyardDominoTiles)}");
            }
        ...
        public override int SelectPlayerWhoWillTakeFirstTurn() //Returns the id of the player playing the first turn, can be 0, 1, 2 or 3.
        {
            //return 0;
            /*if (isSinglePlayerVsIA)
            {*/
            ...
            {
                // Filters the keys whose list has at least one element
        ...
        public override void UpdateScoreAndShowRoundResult(string gameOverCase, int scorePlayer_0, int scorePlayer_1, int scorePlayer_2, int scorePlayer_3, int playerID, bool auxGameIsCompleteAndFinished, string auxPlayerWinner)
        {
            extendedGameController.GameIsCompleteAndFinished = auxGameIsCompleteAndFinished;
            extendedGameController.PlayerWinner = auxPlayerWinner; //Right and Top

            Dictionary<int, string> playersOrderId = new Dictionary<int, string>()
            {
                { 0, "Player" }, // Player
                { 1, "Left" }, // Left AI
                { 2, "Top" }, // Top AI
            ...
            if (!isSinglePlayerVsIA && vsPlayerSelectedID == NumberPlayers.oneVsOne)
            {
                playersOrderId = new Dictionary<int, string>()
                {
                    { 0, "Player" }, // Player
                    { 1, "Top" } // Top AI
                };
        ...
        public override bool CheckRound_GameOver(ref string gameOverCase, bool gameIsBlocked = false)
        {
            ...
            Debug.Log("++**-- gameIsBlocked: " + gameIsBlocked);
        ...
        private string ValideWinnerByTileCount()
        {
            ...
            {
                if (extendedGameController.DeckScript.PlayerTiles.Count + extendedGameController.DeckScript.LeftAITiles.Count == 0)
                    gameOverCase = "Player and Left AI win!";
                if (extendedGameController.DeckScript.TopAITiles.Count + extendedGameController.DeckScript.RightAITiles.Count == 0)
                    gameOverCase = "Top AI and Right AI win!";

                int Team_1_counter = extendedGameController.DeckScript.PlayerTiles.Count + extendedGameController.DeckScript.LeftAITiles.Count;
                int Team_2_counter = extendedGameController.DeckScript.TopAITiles.Count + extendedGameController.DeckScript.RightAITiles.Count;
